...
if __name__ == "__main__":
    ...
    for iters in range(iter_num):
        ...
        KT=0.3*np.ones((bsize,1))#np.random.rand(sample_size,1)*(1-0.01)+0.01
        ...
        domainR=np.random.rand(bsize,1)*(1-0.34)+0.34#np.random.randint(1,7,size=(sample_size,1))*0.4/3+0.2
    ...
    #        array4=ysts4[:,kb,0]/max(ysts4[:,kb,0])
            ...
            if local_peak_n<=1:
                saved_matrix1[kb,:]=np.r_[test_set[kb,5], test_set[kb,12], local_peak_n, mdiv]
            elif 1<local_peak_n<=3:
                saved_matrix2[kb,:]=np.r_[test_set[kb,5], test_set[kb,12], local_peak_n, mdiv]
            else:
                saved_matrix3[kb,:]=np.r_[test_set[kb,5], test_set[kb,12], np.transpose(final_pred), local_peak_n, mdiv]
            #print("kb=%f" % (kb))
            #pdb.set_trace()
        ...
        with open('output/saved_matrix1_13_10000000_613.txt', 'ab') as fd1:
            np.savetxt(fd1, a1)  
            fd1.flush()
        with open('output/saved_matrix2_13_10000000_613.txt', 'ab') as fd2:
            np.savetxt(fd2, a2)
            fd2.flush()
        with open('output/saved_matrix3_13_10000000_613.txt', 'ab') as fd3:
            np.savetxt(fd3, a3)   
            fd3.flush()
